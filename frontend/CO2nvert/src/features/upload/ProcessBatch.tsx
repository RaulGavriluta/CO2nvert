...
import { Loader2, CheckCircle, AlertTriangle, ArrowRight, Zap, Info } from 'lucide-react';
...
const ProcessBatch: React.FC = () => {
  ...
  const handleCalculateEmissions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Redirecționare către Dashboard după succes
        navigate('/');
      } else {
        const errData = await response.json();
      ...
      setError("Eroare la conectarea cu serverul de calcul.");
      setIsCalculating(false);
    }
  };
